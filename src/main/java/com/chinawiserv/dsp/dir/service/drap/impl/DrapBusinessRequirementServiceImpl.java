...
public class DrapBusinessRequirementServiceImpl
    ...
    @Override
    public HandleResult deleteBusinessRequirement(Map<String, ?> paramMap) {
        HandleResult handleResult = new HandleResult();
        try {
            mapper.batchDeleteReq(paramMap);
            mapper.batchDeleteReqRes(paramMap);
            mapper.batchDeleteReqDatasetMap(paramMap);
            handleResult.success("批量删除成功。");
        }
        catch (Exception e){
            logger.error("删除需求失败。",e);
            handleResult.error("删除需求失败。");
