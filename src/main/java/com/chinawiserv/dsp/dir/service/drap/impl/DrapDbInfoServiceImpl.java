...
public class DrapDbInfoServiceImpl extends
    ...
    public void receiveTableChange(Map<String, Object> map) {
        ...
        if (null != map.get("oldTable")) {
            ...
            List<DrapDbTableInfoVo> list = JSON.parseArray(map.get("oldTable").toString(),
                DrapDbTableInfoVo.class);
        ...
        if (null != map.get("newTable")) {
            ...
            //					.get("newTable");
            List<DrapDbTableInfoVo> list = JSON.parseArray(map.get("newTable").toString(),
                DrapDbTableInfoVo.class);
        ...
        if (null != map.get("dicTable")) {
            ...
            List<DrapDictTableInfoVo> listJson = JSON.parseArray(map.get("dicTable").toString(),
                DrapDictTableInfoVo.class);

            List<DrapDictTableInfoVo> list = JSON.parseArray(JSON.toJSONString(listJson),
                DrapDictTableInfoVo.class);

            //删除之前的
            List<String> ids = new ArrayList<>();

            List<DrapDictTableInfo> oldDictTableInfos = dictTableInfoMapper.selectList(
                new EntityWrapper<DrapDictTableInfo>().eq("db_id", list.get(0).getDbId()));
            for (DrapDictTableInfo drapDictTableInfo : oldDictTableInfos) {
                ids.add(drapDictTableInfo.getId());
            }

            dictTableColumnMapper
                .delete(new EntityWrapper<DrapDictTableColumn>().in("table_id", ids));
            dictTableInfoMapper
                .delete(new EntityWrapper<DrapDictTableInfo>().eq("db_id", list.get(0).getDbId()));
    ...
    public void deleteDb(String id) {
        List<String> ids = new ArrayList<>();
        List<DrapDictTableInfo> oldDictTableInfos = dictTableInfoMapper
            .selectList(new EntityWrapper<DrapDictTableInfo>().eq("db_id", id));
        for (DrapDictTableInfo drapDictTableInfo : oldDictTableInfos) {
            ids.add(drapDictTableInfo.getId());
        }
        ...
        ids.clear();
        List<DrapDbTableInfo> tableInfos = dbTableInfoMapper
            .selectList(new EntityWrapper<DrapDbTableInfo>().eq("db_id", id));
    ...
    @Override
    public void synzStatus(String[] ids, String status) {
        DrapDbInfo dbInfo= new DrapDbInfo();
        for (String id : ids) {
            dbInfo.setId(id);
            dbInfo.setStatus(Integer.parseInt(status));
            mapper.updateById(dbInfo);
        }
    }
