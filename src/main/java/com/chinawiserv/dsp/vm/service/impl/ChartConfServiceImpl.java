...
public class ChartConfServiceImpl extends CommonServiceImpl<ChartConfMapper, ChartConf, ChartConfVo> implements IChartConfService {
	...
	public boolean insertUsersChart(Map<String, Object> paramsMap) throws Exception {
		if (paramsMap.isEmpty()) {
			...
			chartConf.setChartName(paramsMap.getOrDefault("chart_name", "").toString());// 图表标题
			chartConf.setIsNameShow(Integer.parseInt(paramsMap.getOrDefault("is_name_show", "0").toString()));// 标题是否显示
			chartConf.setChartDesc(paramsMap.getOrDefault("chart_desc", "").toString());// 图表描述
			chartConf.setChartType(paramsMap.getOrDefault("chart_type", "").toString());// 图表类型
			chartConf.setChartUrl("");// 图表URL
			chartConf.setChartTimeType(paramsMap.getOrDefault("chart_time_type", "").toString());// 图表时间范围类型
			chartConf.setChartTimeScope(paramsMap.getOrDefault("chart_time_scope", "").toString());// 图表时间范围取值
			chartConf.setStatus(1);// 状态
			chartConf.setCreateUserId(ShiroUtils.getLoginUserId());// 创建人
			chartConf.setCreateTime(new Date());// 创建时间
			chartConf.setHasSubIndictor(Integer.parseInt(paramsMap.getOrDefault("has_sub_indictor", "0").toString()));
			...
			chartClassify.setChartType(paramsMap.getOrDefault("chart_type", "").toString());// 对应图形类型

			if (1 == chartConf.getHasSubIndictor()) {
				// 图表描述与指标关系表
				chartDescIndictorMap.setId(CommonUtil.get32UUID());
				chartDescIndictorMap.setChartId(chartId);
				chartDescIndictorMap.setIndictorCode(paramsMap.getOrDefault("sub_indictor_code", "").toString());// 指标编号
				chartDescIndictorMap.setIndictorShowName(paramsMap.getOrDefault("sub_indictor_show_name", "").toString());// 指标显示名称
				// 图表指标描述信息
				chartDescription.setId(CommonUtil.get32UUID());
				chartDescription.setChartId(chartId);
				chartDescription.setSubTitle(paramsMap.getOrDefault("sub_title", "").toString());// 副标题
			...
			chartMenuCustom.setShowOrder(Integer.parseInt(paramsMap.getOrDefault("is_name_show", "0").toString()));// 图表显示顺序
