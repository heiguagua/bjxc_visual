...
public class LoginController extends BaseController {
	...
	public String login(String return_url, Model model) throws UnsupportedEncodingException {
		/**
		 * 自留代码 String index = "/catalog/catalogue"; model.addAttribute("return_url",
		 * StringUtils.isNotBlank(return_url) ? URLDecoder.decode(return_url, "UTF-8") :
		 * index); return "login";
		 **/
		String index = "/chartMenuTemplate/index";
		...
		// wrapper.where("setting_code={0} or setting_code={1} or setting_code={2}",
		// "systemName", "systemShortName", "systemSubName","projectPortalName");
		...
		// }else if(!StringUtils.isEmpty(sysNameMap.get("systemName")) &&
		// !StringUtils.isEmpty(sysNameMap.get("systemSubName"))){
		// indexLogoName = sysNameMap.get("systemName") + "-" +
		// sysNameMap.get("systemSubName");
		// }
		...
		// model.addAttribute("indexLogoName", sysNameMap.get("systemName") + "-" +
		// sysNameMap.get("systemSubName"));
	...
	public HandleResult doLogin(HttpServletRequest request, @RequestParam Map<String, Object> paramMap, Model model, RedirectAttributes redirectAttributes) {
		...
		try {
			 ...
			 * 记录登录日志
			 */
			// sysLogService.insertLog("用户登录成功", ShiroUtils.getLoginUser(),
	...
	private void loginSuccess(Map<String, Object> paramMap, HttpServletRequest request) {
		...
		try {
			...
			// String systemName = ShiroUtils.getSessionAttribute("systemName") == null ? ""
			// : ShiroUtils.getSessionAttribute("systemName").toString();
			// String systemSubName = ShiroUtils.getSessionAttribute("systemSubName") ==
			// null ? "" : ShiroUtils.getSessionAttribute("systemSubName").toString();
			// String systemShortName = ShiroUtils.getSessionAttribute("systemShortName") ==
			// null ? "" : ShiroUtils.getSessionAttribute("systemShortName").toString();
			// String projectPortalName =
			// ShiroUtils.getSessionAttribute("projectPortalName") == null ? "" :
			// ShiroUtils.getSessionAttribute("projectPortalName").toString();
			//
			...
			// !StringUtils.isEmpty(systemSubName)){
			...
			// sysMenuService.selectTreeMenuByUserId(currentLoginUser.getId());
			 ...
			 * sysMenuService.selectMenuIdsByuserId(currentLoginUser.getId()); String[]
			 * permissions = list2.toArray(new String[list2.size()]);
